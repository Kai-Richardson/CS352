...
int main(int argc, char** argv) {
    ...
    //
    // Read simulation parameters from command line/environment
    //
    ...
    for (long iteration = 0; iteration < num_iterations; ++iteration) {
        ...
        #pragma omp parallel
        {
            ...
            // PRNG for each thread, using threadnum
            // Was thread-safe before, but you asked for this specifically
            std::mt19937 rand{};
            rand.seed(omp_get_thread_num());
            ...
            my_sim_res.push_back(total_savings);
        }

        // save simulation results
        #pragma omp critical(insertion)
        simulation_results.insert(std::end(simulation_results), std::begin(my_sim_res), std::end(my_sim_res));
